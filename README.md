...
| **Log Relative Tolerance** | `10^x` relative tolerance (rtol) for adaptive methods (`ae_*` / `se_*`). |
| **Log Absolute Tolerance** | `10^x` absolute tolerance (atol) for adaptive methods (`ae_*` / `se_*`). |
...
|---|---|---|---|
| **Log Relative Tolerance** | −3.0 | −7.0 〜 0.0 | `10^x` rtol for `ae_*` / `se_*`. Smaller = more precise, slower. Must be ≥ the Log Absolute Tolerance. |
| **Log Absolute Tolerance** | −4.0 | −7.0 〜 0.0 | `10^x` atol for `ae_*` / `se_*`. Smaller = more precise, slower. Must be ≤ the Log Relative Tolerance. |
...
| Parameter | Default | Range | Description |
|---|---|---|---|
| **Min Sigma** | 0.00001 | 0.0 〜 0.1 | Sigma below which ODE gradient is treated as zero. Normally unchanged. |
...
| **Log Relative Tolerance** | 可変ステップメソッド (`ae_*` / `se_*`) の `10^x` 相対許容誤差（rtol）。 |
| **Log Absolute Tolerance** | 可変ステップメソッド (`ae_*` / `se_*`) の `10^x` 絶対許容誤差（atol）。 |
| **Max ODE Steps** | 可変ステップメソッドのステップ数上限。 |
...
| パラメータ | デフォルト | 範囲 | 説明 |
|---|---|---|---|
| **Log Relative Tolerance** | −3.0 | −7.0 〜 0.0 | `ae_*` / `se_*` の `10^x` rtol。小さいほど高精度・低速。Log Absolute Tolerance 以上にする。 |
| **Log Absolute Tolerance** | −4.0 | −7.0 〜 0.0 | `ae_*` / `se_*` の `10^x` atol。小さいほど高精度・低速。Log Relative Tolerance 以下にする。 |
| **Max ODE Steps** | 1000 | 10 〜 5000 | 可変ステップメソッドのステップ数上限（フェイルセーフ）。`fe_*` には影響なし。 |

### 詳細設定

| パラメータ | デフォルト | 範囲 | 説明 |
|---|---|---|---|
| **Min Sigma** | 0.00001 | 0.0 〜 0.1 | これ以下の sigma で ODE 勾配をゼロとみなす。通常は変更不要。 |
| **PID P** | 0.0 | 0.0 〜 1.0 | 可変ステップコントローラの比例係数（`ae_*` のみ）。 |
